...
with tf.compat.v1.name_scope('input'):
    input_data = tf.compat.v1.placeholder(dtype=tf.float32, shape=(None, input_size, input_size, 3), name='input_data')
model = YOLOV3(input_data, trainable=False)
load_ops = load_weights(tf.compat.v1.global_variables(), darknet_weights)
