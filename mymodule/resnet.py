...
def subsample(inputs,factor,scope=None):
    ...
    if factor==1:
        ...
        return tf_slim.max_pool2d(inputs,kernel_size=[1,1],stride=factor,scope=scope)
 
#定义卷积操作方法
def conv2d_same(inputs,num_outputs,kernel_size,stride,scope=None):
    if stride==1:
        return tf_slim.conv2d(inputs,num_outputs,kernel_size,stride=1,padding='SAME',scope=scope)
    else:
        #修整Inputs,对inputs进行补零操作
        padding_total=kernel_size-1
        padding_beg=padding_total//2
        padding_end=padding_total-padding_beg
        inputs=tf.pad(tensor=inputs,paddings=[[0,0],[padding_beg,padding_end],[padding_beg,padding_end],[0,0]])
        return tf_slim.conv2d(inputs,num_outputs,kernel_size,stride=stride,padding='VALID',scope=scope)
...
def resnet_arg_scope(is_training=True,
    ...
    with tf_slim.arg_scope(
        ...
        normalizer_fn=tf_slim.batch_norm,
        normalizer_params=batch_norm_params ):
        with tf_slim.arg_scope([tf_slim.batch_norm],**batch_norm_params):
            with tf_slim.arg_scope([tf_slim.max_pool2d],padding='SAME') as arg_sc:
                return arg_sc
...
def bottleneck(inputs,depth,depth_bottleneck,stride,outputs_collections=None,scope=None):
    with tf.compat.v1.variable_scope(scope,'bottleneck_v2',[inputs]) as sc:
        ...
        preact=tf_slim.batch_norm(inputs,activation_fn=tf.nn.relu,scope='preact')
        if depth==depth_in:
            #如果残差单元输入通道和输出通道数一样，就对inputs进行降采样
            shortcut=subsample(inputs,stride,'shortcut')
        else:
            #如果残差单元输入通道与输出通道数不一样，就使用stride步长的1*1卷积改变其通道数,输入通道和输出通道数一样
            shortcut=tf_slim.conv2d(preact,depth,[1,1],stride=stride,normalizer_fn=None,activation_fn=None,scope='shortcut')
        #定义残差
        #第一步：1*1，stride=1，输出通道数为depth_bottleneck的卷积
        #第二步：3*3，stride=stride，输出通道数为depth_bottleneck的卷积
        #第三步：1*1，stride=1，输出通道数为depth的卷积
        residual=tf_slim.conv2d(preact,depth_bottleneck,[1,1],stride=1,scope='conv1')
        residual=tf_slim.conv2d(residual,depth_bottleneck,[3,3],stride=stride,scope='conv2')
        residual=tf_slim.conv2d(residual,depth,[1,1],stride=1,scope='conv3')
...
def resnet_v2(inputs,
    ...
    with tf.compat.v1.variable_scope(scope,'resnet_v2',[inputs],reuse=reuse) as sc:
        ...
        with tf_slim.arg_scope([tf_slim.conv2d,bottleneck,stack_block_dense],outputs_collections=end_point_collections):
            net=inputs
 
            if include_root_block:
                #根据include_root_block标记，创建resnet最前面一层的卷积神经网络
                with tf_slim.arg_scope([tf_slim.conv2d],activation_fn=None,normalizer_fn=None):
                    net=conv2d_same(net,64,7,stride=2,scope='conv1')
                net=tf_slim.max_pool2d(net,[3,3],stride=2,scope='pool1')
            #利用stack_blocks_dense将残差学习模块完成
            net=stack_block_dense(net,blocks)
            net=tf_slim.batch_norm(net,activation_fn=tf.nn.relu,scope='postnorm')
            ...
            if num_classes is not None:
                ...
                net=tf_slim.conv2d(net,num_classes,[1,1],activation_fn=None,normalizer_fn=None,scope='logits')
            ...
            if num_classes is not None:
                #添加一个softmax输出层
                end_points['prediction']=tf_slim.softmax(net,scope='prediction')
...
def resnet_v2_50(inputs,
    ...
    #前3个blocks包含步长为2的层，总尺寸244/(4*2*2*2)=7 输出通道变为2048
    blocks=[
...
if __name__=='__main__':
    ...
    with tf_slim.arg_scope(resnet_arg_scope(is_training=False)):
        net, end_points = resnet_v2_50(inputs, 11)
    ...
    #print(sess.run(end_points['prediction']))
    print(sess.run(end_points['resnet_v2_50/logits']))
